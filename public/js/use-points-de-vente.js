...
export function usePointsDeVente({ showAlert, allPointsDeVente }) {
  ...
  async function addPointDeVente() {
    ...
    try {
      const pdv = await apiFetch('/points-de-vente', {
        method: 'POST',
        body: JSON.stringify({ nom: newPdvNom.value }),
      });
  ...
  async function saveEditPdv() {
    ...
    try {
      const pdv = await apiFetch(`/points-de-vente/${pdvEditId.value}`, {
        method: 'PATCH',
        body: JSON.stringify({ nom: pdvEditNom.value }),
      });
  ...
  return {
    pdvLoading, newPdvNom, pdvSaving, pdvEditId, pdvEditNom,
  ...
  <div v-else style="display:flex;flex-direction:column;gap:6px">
    <div v-for="pdv in allPointsDeVente" :key="pdv.id"
      ...
      <template v-else>
        ...
        <span style="font-size:14px;font-weight:500;flex:1;color:#1e293b">{{ pdv.nom }}</span>
        <button class="btn-sm" style="background:#f8fafc;color:#64748b;border:1px solid #e2e8f0" @click="startEditPdv(pdv)">✏ Renommer</button>
